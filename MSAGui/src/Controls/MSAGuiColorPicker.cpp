...
#include "MSAGuiColorPicker.h"

namespace MSA {
	
	GuiColorPicker::GuiColorPicker(string name, float* value, float max) : GuiControl(name) {
		this->value = value;
		this->min	= 0;
		this->max	= max;
	...
	void GuiColorPicker::loadFromXML(ofxXmlSettings &XML) {
		for(int i=0; i<4; i++) {
			setValue(XML.getValue(controlType + "_" + key + ":values_" + ofToString(i), 0.0f), i);
		}
	}
	
	void GuiColorPicker::saveToXML(ofxXmlSettings &XML) {
	...
	float GuiColorPicker::getValue(int i) {
		return (value)[i];
	}
	
	
	void GuiColorPicker::setValue(float f, int i) {
		if(f < min) f = min;
		else if(f > max) f = max;
		(value)[i] = f;
	}
	...
	void GuiColorPicker::updateSlider() {
		if(!enabled) return;
	...
	void GuiColorPicker::onPress(int x, int y, int button) {
		updateSlider();
	}
	
	void GuiColorPicker::onDragOver(int x, int y, int button) {
		updateSlider();
	}
	
	void GuiColorPicker::onDragOutside(int x, int y, int button) {
		updateSlider();
	}
	...
	//--------------------------------------------------------------------- draw
	void GuiColorPicker::draw(float x, float y) {
