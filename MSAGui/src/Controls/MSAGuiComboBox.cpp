...
#define kMaxNameStringLen 100
	...
	GuiComboBox::GuiComboBox(string name, int &choice_out, int numChoices, GuiPage *owner, string* choiceTitles ) :
	GuiControl(name),
	...
	void GuiComboBox::setTitleForIndex(int index, string title) {
		if(index < 0 || index >= m_choices.size()) return;
		m_choices[index] = title;
	}
	
	string GuiComboBox::getTitleForIndex(int index) {
	...
	void GuiComboBox::addChoice(string title, int index) {
	...
	int GuiComboBox::getIndexForTitle(string title) {
	...
	void GuiComboBox::removeChoice(string title) {
		int index = getIndexForTitle(title);
		if(index >= 0) removeChoice(index);
	}
	
	void GuiComboBox::removeChoice(int index) {
	...
	void GuiComboBox::loadFromXML(ofxXmlSettings &XML) {
		setValue(XML.getValue(controlType + "_" + key + ":value", 0));
	}
	
	void GuiComboBox::saveToXML(ofxXmlSettings &XML) {
	...
	void GuiComboBox::keyPressed( int key ) {
	}
	
	int GuiComboBox::getValue() {
		return m_selectedChoice;
	}
	
	void GuiComboBox::setValue(int index) {
		m_selectedChoice = ofClamp(index, 0, m_choices.size());
	}
	
	void GuiComboBox::setValue(string title) {
	...
	void GuiComboBox::onPress(int x, int y, int button) {
		//	beenPressed = true;
	...
	void GuiComboBox::onPressOutside(int x, int y, int button){
		if(m_hasFocus)
			releaseEventStealingFocus();
	}
	
	
	void GuiComboBox::onDragOver(int x, int y, int button){
		//same behavior as mouse move
		onMouseMove(x,y);
	}
	
	void GuiComboBox::onDragOutside(int x, int y, int button){
		//same behavior as mouse move
		onMouseMove(x,y);
	}
	
	bool GuiComboBox::hitTest(int tx, int ty) {
		if(!m_hasFocus) 
	...
	void GuiComboBox::onMouseMove(int x, int y) {
	...
	void GuiComboBox::onReleaseOutside(int x, int y, int button) {
		onRelease(x, y, button);
	}
	
	void GuiComboBox::onRelease(int x, int y, int button) {
		if(m_hasFocus && m_mouseMovedSinceClick) {
			releaseEventStealingFocus();
		}
	}
	
	void GuiComboBox::releaseEventStealingFocus(){
	...
	//special overloads - this is a hack - later think about making GuiControl's methods virtual.
	void GuiComboBox::setCBTextColor() {
		if(m_hasFocus) ofSetColor(config->textOverColor);
		else ofSetColor(config->textColor);
	}
	
	void GuiComboBox::setCBTextBGColor() {
...
#define kSGCBTextPaddingY    15
	void GuiComboBox::draw(float x, float y) {
