...
void Particle::update() {
	...
	int fluidIndex = myApp->fluidSolver.getIndexForPos(Vec2f(x * myApp->window.invWidth, y * myApp->window.invHeight));
	MSA::Vec3f fluidVel = myApp->fluidSolver.getVelocityAtPos(fluidIndex);
	vx = fluidVel.x * myApp->window.width * mass * FLUID_FORCE + vx * MOMENTUM;
	vy = fluidVel.y * myApp->window.height * mass * FLUID_FORCE + vy * MOMENTUM;
