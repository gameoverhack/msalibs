...
#include "MSAGuiQuadWarp.h"

namespace MSA {
#define MOUSE_DISTANCE		10.0f
	
	//---------------------------------------------------------------------
	GuiQuadWarp::GuiQuadWarp(string name, ofBaseDraws &baseDraw, ofPoint *pts) : GuiControl(name) {
	...
	void GuiQuadWarp::setup() {
		
	}
	
	
	void GuiQuadWarp::loadFromXML(ofxXmlSettings &XML) {
		for(int i=0; i<4; i++) {
	...
	void GuiQuadWarp::saveToXML(ofxXmlSettings &XML) {
	...
	//---------------------------------------------------------------------
	void GuiQuadWarp::onPress(int x, int y, int button) {
		curPoint = NULL;
		for(int i=0; i<4; i++) {
	...
	void GuiQuadWarp::onPressOutside(int x, int y, int button) {
		onPress(x, y, button);
	}
	
	
	void GuiQuadWarp::onDragOver(int x, int y, int button) {
		if(curPoint) {
	...
	//---------------------------------------------------------------------
	void GuiQuadWarp::onDragOutside(int x, int y, int button) {
	...
	//---------------------------------------------------------------------
	void GuiQuadWarp::draw(float x, float y) {
		setPos(x, y);
		glPushMatrix();
		glTranslatef(x, y, 0);
		glColor3f(1, 1, 1);
