...
#include "MSAGuiSlider2d.h"


namespace MSA {
	GuiSlider2d::GuiSlider2d(string name, ofPoint& value, float xmin, float xmax, float ymin, float ymax) : GuiControl(name) {
		min.set(xmin, ymin);
		max.set(xmax, ymax);
		this->value = &value;
	...
	void GuiSlider2d::setValue(float x, float y) {
		(*value).x = x;
		(*value).y = y;
	}
	
	void GuiSlider2d::setMin(float x, float y) {
		min.x = x;
		min.y = y;
	}
	
	void GuiSlider2d::setMax(float x, float y) {
		max.x = x;
		max.y = y;
	}
	
	void GuiSlider2d::onPress(int x, int y, int button) {
		lock = true;
		point.set(x, y);
	}
	
	void GuiSlider2d::onDragOver(int x, int y, int button) {
		if(lock) {
			point.set(x, y);
		}
	}
	
	void GuiSlider2d::onDragOutside(int x, int y, int button) {
		if(lock) {
			point.set(x, y);
		}
	}
	
	void GuiSlider2d::onRelease() {
		lock = false;
	}
	
	void GuiSlider2d::update() {
	...
	void GuiSlider2d::draw(float x, float y) {
		setPos(x, y);
		ofPoint	pointv;
